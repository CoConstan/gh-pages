...
void Glouton(int *Combinaison, int n, double *Poids, double *Prix)
{
	...
	//rmq 1 : Ecrire une fonction de trie n'est pas obligatoire. On peut utiliser qsort 
	//de la librairie stdlib.h pour trier notre tableau de ratio. 

	//rmq 2 : la complexité de cette algo dépends entièrement de la fonction de tri utilisé. Ici nous avons utilisé un
	//tri à bulle, de complexité O(n²) au maximum et en moyenne. Une optimisation possible est donc d'utiliser
	//un meilleurs tri comme le "quicksort" ou le "tri à fusion" ayant une complexité en O(nlog(n)) dans la grande
	// majorité des cas.

	//rmq 3 : La solution depends aussi du tri utlisé mais sera toujours un resultat valide et proche de la
